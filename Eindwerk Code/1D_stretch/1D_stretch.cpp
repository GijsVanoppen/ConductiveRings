...
The ratio r/a_iterations has to be large enough! (1/500 works, 1/1000 doesnt)
...
tuple<int, int, double, double, double> handle_input(string input_file_name){
    ...
    auto parameters = make_tuple(N, a_iterations, r, R, R_j);
    input_file.close();
    return parameters;
}
...
int main(){
    ...
    double R_tot;   //total resistance of the circuit
    double R_1;     //first resistance (used in calculating R_tot)
    double R_2;     //second resistance (used in calculating R_tot)

    auto parameters = handle_input("1D_stretch_input.txt");
    int N = get<0>(parameters);
    int a_iterations = get<1>(parameters);
    double r = get<2>(parameters);
    double R = get<3>(parameters);
    double R_j = get<4>(parameters);

    double a_delta = r/((double)a_iterations+1); 
    ...
    write_results_to_file("1D_stretch_results.txt",results);
