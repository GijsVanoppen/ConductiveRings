...
def draw_circles(file_name):
    ...
    for line in f:
        ...
        circle_index = int(line[0:space_index])
        circle_x = float(line[space_index+1:space_index2])
        circle_y = float(line[space_index2+1:space_index3])
        r = float(line[space_index3+1:space_index4])
        R = float(line[space_index4+1:-1])
        c = plt.Circle((circle_x, circle_y), radius = r, fill= False)
        plt.text(circle_x,circle_y, str(circle_index)) 
